...
top_three_recommendations(Purpose, Climate, Budget, Duration, Transport, Crowd) :-
    best_trip(Purpose, Climate, Budget, Duration, Transport, Crowd, [A,B,C|_]),
    write(A), nl,
    write(B), nl,
    write(C), nl, !.
...
avoid_crowded_places(Destination) :-
    ...
    write(Destination),
    write(' may be crowded'),
    nl,
    fail.
avoid_crowded_places(_).

travel_advice(manali) :- write('Carry woolen clothes'), !.
travel_advice(goa) :- write('Carry sunscreen'), !.
travel_advice(ladakh) :- write('Carry oxygen support'), !.
travel_advice(_).
